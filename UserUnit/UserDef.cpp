...
#pragma region USERDEF
    ...
    {
        this->registerUser(newUserName, newPassword, newLevel);
    }
    ...
    void User::setLevel(User& admin, string newLevel)
    {
        if (admin.capable(UserLevel::Administor))
        {
            if (newLevel == "guest")
                this->Level = UserLevel::Guest;
            else if (newLevel == "user")
                this->Level = UserLevel::Visitor;
            else if (newLevel == "operator")
                this->Level = UserLevel::Operator;
            else if (newLevel == "administor")
                this->Level = UserLevel::Administor;
        }
    }
...
#pragma endregion
#pragma region GuestDef
    Guest::Guest(const char* newUserName, const char* newPassword)
        :User(newUserName,newPassword,UserLevel::Guest)
    {}
    Guest::Guest(const Guest& obj)
        :User(obj)
    {}
    void Guest::promote(const User& executor)
    {
        ;
    }
#pragma endregion
...
#pragma region USERLISTDEF
    ...
    //获取用户的索引
    int UserList::getIndex(string userName)
    {
        for (int n = 0; n < allUser.getSize(); ++n)
        {
    ...
    {
        int result = getIndex(userName);
        if (result == -1)
            return *(User*)ZERO(sizeof(User)).Export();
        else
            return allUser[result];
    }
    void UserList::removeUser(string userName)
    {
        allUser.remove(this->getIndex(userName));
    }
    void UserList::changeInfor(string userName, string oldPassword, string newPassword, User* admin, UserLevel newLevel)
    {
        ...
        {
            ErrorInfor::instance.write("User not exist");
        }
    }
}
#pragma endregion
